...
## 0.5. 背景 — なぜ「30秒待機」から「バックエンドのコールドスタート」に主題が移ったか

### 0.5-1. 出発点

本セッションの当初のミッションは
**「Chatty-sp の初期起動問題（30秒待機と2重発火）を根本的に解決すること」**。

2026年3月の実装当時、初期起動時のリップシンク不具合を解決できず、
以下の2つを**暫定処置**として入れたまま運用してきた経緯がある。

| 暫定処置 | 場所 | 当時の理解 |
|---|---|---|
| 初期挨拶の30秒待機ゲート | `live_api_handler.py:754-764` | アバター準備完了を待ってから挨拶させるため |
| 初期起動時に「リロードのフロー」を二重に流す | `core-controller.ts:94` と `:106` | リロード時は不具合が出ないことを発見しての苦肉の策 |

**当初の仮説：この2つが初期起動の遅さの元凶であり、外せば速くなる。**

α版テストへ進むにあたり、この暫定処置を抜本的に解消するのが本セッションの目的だった。

### 0.5-2. 仮説が覆るまでの3段階

#### 段階1 — コード精読による論証（実行前）

「まず回してみて結果で直す」を避け、コードを先に精読した。その結果：

**(a) ユーザーが聞く挨拶は30秒ゲートを通っていない**

`resetAppContent()` が2回呼ばれることで LiveAPI セッションが2つ作られる。

| | セッションA（1回目） | セッションB（2回目） |
|---|---|---|
| 分岐条件 | `session_count == 1` | `session_count` 1→2 |
| 入るブランチ | 初回挨拶ブランチ `live_api_handler.py:755-764` | 再接続ブランチ `:780` |
| **30秒ゲート** | **通る** | **通らない** |
| `_is_initial_greeting_phase` | `True` | `False` |
| 最期 | `live_stop` で `stop()` される | 生き残る |
| **ユーザーに聞こえるか** | **聞こえない** | **これが挨拶** |

**(b) セッションAの出力は物理的に届かない**

`stop()` は `is_running = False` を立てるだけ（`live_api_handler.py:689-692`）。
そして音声を転送するループの条件は：

```python
# live_api_handler.py:905
while not self.needs_reconnect and self.is_running:
```

停止済みのセッションAは30秒後にダミー発話を送るが、このループが回らないため
**音声は1バイトもクライアントに転送されない。**

**(c) 「2重あいさつ」は起きていない**

emit は21箇所すべて `room=self.client_sid` を指定している。
sid が異なる＝別接続なので、同一ユーザーが挨拶を2回聞くことはない。
（当初「2重あいさつが起きている」と報告したが、これは誤りだったので訂正した）

**(d) 30秒ゲートの設計意図は一度も機能していない**

`greeting_trigger` を送るのは `concierge-controller.ts:71`（`linkLamAvatar()` 内）で、
...
`greeting_trigger` はセッションBのオブジェクトに届くため、
**セッションAの `_greeting_trigger_event` は永遠にセットされない。**
つまり「アバター準備完了を待ってから挨拶」という設計は、実装当初から一度も動いていない。

> **この時点での中間結論**
> 30秒ゲートは毎回フル30秒空回りしている（＝無駄という指摘は正しい）が、
> **ユーザーの体感時間への寄与は 0秒**（＝遅さの犯人ではない）。

#### 段階2 — 実機ログによる主因の特定

ユーザーが実機で起動し、アバター表示まで約45秒を体感した際の Cloud Run ログを取得。
...
12:01:58  Default STARTUP TCP probe succeeded after 1 attempt for
          ...
          ↓ ここから35.3秒、import / GCS読み込み / TTS×4 が続く
12:02:33  （アプリ初期化完了）
12:02:40  （挨拶発話・アバター表示）
```

`Starting new instance. Reason: AUTOSCALING` が明示的に出ており、
**コンテナ `chatty-sp-base-1` が新規に起動している＝コールドスタートである。**

#### 段階3 — コールド／ウォーム比較による決定的な検証

同一フローをコールド時とウォーム時で計測し、区間ごとに比較した。

| 区間 | 内容 | **コールド** | **ウォーム** | 差 |
|---|---|---|---|---|
| **区間A** | インスタンス起動 → アプリ初期化完了 | **35.3秒** | **0秒** | **−35.3秒** |
| **区間B** | WebSocket接続 → 挨拶発話 | **7.27秒** | **7.76秒** | **+0.49秒** |
| 合計 | | 約45秒 | 約8秒 | −37秒 |

**この表が決定的な検証データである。**

- 30秒ゲートを含むアプリ側の処理はすべて**区間B**で起きる
- その区間Bは **コールド 7.27秒 / ウォーム 7.76秒 とほぼ同一**（差 0.49秒、誤差範囲）
- 45秒と8秒の差 37秒は、**ほぼ全量が区間A（コールドスタート）に由来**

→ **30秒ゲートも2重発火も、体感時間の差を作っていない。**
→ **主因は Cloud Run のコールドスタート（35.3秒、全体の78%）。**
...
| 区間 | 秒数 | 内容 |
|---|---|---|
| gunicorn 起動 | 1.1s | ポートバインドまで |
| import群 → `support_core.py:16` | 13.8s | flask / socketio / google.genai / **google.generativeai（legacy）** など |
| GCSプロンプト読み込み（1回目） | 5.2s | `exists()` + `download_as_text()` の往復 |
| **`live_api_handler` import** | **14.3s** | **scipy/numpy + TTS `synthesize_speech()` を4回、直列でネットワーク往復** |
| GCSプロンプト読み込み（2回目・重複） | 0.8s | `app_customer_support.py:126`（`support_core.py:165` と重複） |
| **合計** | **35.3s** | |

補足：14.3秒の区間に `[CachedAudio]` のログが出ないのは、
`logging.basicConfig()` が `app_customer_support.py:47` にあり、
`from live_api_handler import ...`（`:45`）**より後**に実行されるため。

### 0.5-4. 途中で否定された仮説（同じ轍を踏まないための記録）

| 仮説 | 否定した根拠 |
|---|---|
| 「毎回30秒待たされている」 | 30秒は固定待機ではなく**上限30秒のタイムアウト**。かつ体感への寄与は0秒（§0.5-2） |
| 「2重あいさつが聞こえている」 | emit は21箇所すべて `room=self.client_sid`。sid が違えば別接続で、同一ユーザーには届かない |
| 「2重発火は単なるバグ」 | 逆。セッションBが再接続ブランチに入り `_is_initial_greeting_phase = False` になることで、壊れている `_send_a2e_ahead()` の一括送信パスを回避している。**これがリップシンクを成立させている唯一の仕組み** |
| 「`--min-instances=1` だからコールドスタートは起きない」 | 実際は全ブランチで `--min-instances=0`。かつログに `AUTOSCALING` による新規インスタンス起動が記録されている |
| 「A2E がコールドスタートしている」 | 同時間帯の `audio2exp-onnx` は5チャンクを1.9秒で処理（全て200 OK）＝ウォーム。コールドスタートしたのは `chatty-sp-base` のみ |
| 「未使用依存（anthropic / pyiceberg / firestore）を削れば起動が速くなる」 | これらはどこからも import されていない。イメージサイズには効くが**起動時間には効かない** |
| 「GCS のプロンプト読み込みが重い」 | ログでは6ファイル 0.28秒。当初「48往復で重い」と述べたのは過大評価。実削減見込みは約2.6秒 |
| 「visit_count が二重カウントされている」 | Supabase の DNS 解決自体が失敗（`[Errno -2] Name or service not known`）しており、書き込みは1回も起きていない |

### 0.5-5. そこから統合検討に至った流れ

主因がコールドスタートと確定したため、`--min-instances=1`（常駐化）を検討した。

1. **目標をゼロに設定** — ユーザーの指示は「目標は0＝すなわちコールドスタートを止める」
2. **コストを試算** — 1サービスあたり 月 $8.37〜9.72
3. **3サービスに横展開すると 月 $27.81** — Chatty / Travel / MTG は同一 GCP プロジェクトで無料枠を共有するため
4. **ユーザーの指摘** —
   > 「それで意味がない！、同じような処理なら共有化できるかを検討してからでしょ？」
5. **3リポジトリのコードを実測比較** — `api_integrations.py` と `long_term_memory.py` は0行差、
   Chatty は Travel の完全なスーパーセット（§4-1）
6. → **統合すれば1サービス分（月 $8.37）で済み、かつ修正の検証も1回で済む**（§9）

**この文書は、5〜6 の具体的な設計とプランニングを引継ぎ先に依頼するために作成した。**
...
> §0.5-2 と内容が一部重複するが、あちらは「主因特定に至った経緯」、
> こちらは「修正を設計するための事実の一覧」として意図的に両方に記載している。
