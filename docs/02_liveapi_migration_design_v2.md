...
| `speakTextGCP()` | ショップ説明に使用 | **ショップ説明には使用しない**（フォールバック用のみ残す） |
...
- `handleShopSearchFromLiveAPI()` 内の `speakTextGCP()` 呼び出し
- `sendMessage()` 内のショップ説明TTS読み上げロジック（`ttsIntro` + `shopLines` 分割再生）
- ショップ説明における `isUserInteracted` チェック

#### 4.6.2 維持するもの

- `speakTextGCP()` メソッド自体 → フォールバック用に残す
- `isUserInteracted` / `enableAudioPlayback()` → テキスト入力時のTTS等で引き続き使用
- `ttsPlayer`（HTMLAudioElement）→ フォールバック用に残す
...
`live_fallback` でREST APIモードに切り替え、既存の `speakTextGCP()` で読み上げる。
