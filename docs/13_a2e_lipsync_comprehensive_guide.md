...
### 1.5 LiveAPI 0.1秒チャンクがA2Eで失敗する根本原因

REST/ショップ説明では正常にリップシンクするのに、LiveAPIのストリーミングでは「もごもご」「口閉じ固定」「19秒間止まらない」が発生する。その原因は以下の3つ。

#### 1.5.1 モデルの前後参照（コンテキスト）の欠如

A2E（Wav2Vec）は、ある瞬間の音だけでなく**前後の音の流れ（文脈）**を見て口の形を決定する。

| パス | 挙動 |
|------|------|
| REST / ショップ説明（成功） | 文章全体を渡すため、モデルは「言葉の始まりから終わりまで」を把握し、滑らかな表情を生成 |
| LiveAPI 0.1秒チャンク（失敗） | 「おはよ…」の「お」だけでは「おはよう」か「おやすみ」か判断不能。自信がないため「口をあまり動かさない（もごもご）」を返す |

#### 1.5.2 時計のズレ（ドリフト）の蓄積

これが「19秒間止まらない」問題の正体。

| パス | 挙動 |
|------|------|
| REST / ショップ説明（成功） | 1回の再生ごとに時計をリセット。多少ズレても数秒で再生完了するため問題にならない |
| LiveAPI（失敗） | 最初の発話から最後まで同じ時計（`performance.now`）を使い続ける。1チャンクごとに数ミリ秒ずつ発生する推論待ち時間やネットワーク揺らぎが雪だるま式に蓄積し、**「音はもう終わったのに、計算上の時計だけが未来を走っている」**状態になる |

#### 1.5.3 チャンク繋ぎ目の不連続性

| パス | 挙動 |
|------|------|
| REST / ショップ説明（成功） | 繋ぎ目がない。1つの連続した音声として処理 |
| LiveAPI（失敗） | 0.1秒ごとに「口を閉じた状態」から始まる推論結果を無理やり繋げている。これがカクツキや「口閉じ固定」の原因 |

### 1.6 解決方針：「成功しているショップ説明のロジック」をLiveAPI会話に逆輸入する

**「LiveAPIだからリアルタイムに0.1秒で処理しなきゃ」という発想が、RESTなら起きなかった問題を招いていた。**

Gemini LiveAPIから流れてくる細かな音声は**そのまま再生**しつつ、A2E（表情）の処理だけを、成功実績のある「ショップ説明」と同じくらいの**意味のある塊（セグメント）**単位にまとめる。

具体的な方向性：
1. **0.1秒でのフラッシュを廃止** — 最低でも「句読点」や「0.5〜1.0秒」単位でA2Eに投げる
2. **同期基準の変更** — 暴走しやすい `performance.now` 連続計測から、セグメント単位の管理に戻す
3. **「RESTでできていたこと」をLiveAPIの上で再現する** — 実装を複雑にするのではなく、成功パターンをメインの会話にも適用する

**注意**: この方針はA2Eへの投入単位の変更であり、音声再生側（`playPcmAudio`）は変更しない。音声はLiveAPIからの到着順にそのまま再生し続ける。
