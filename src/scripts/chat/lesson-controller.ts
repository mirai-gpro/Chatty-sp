...
export class LessonController extends CoreController {
  ...
  private sendGreetingTrigger(reason: 'ready' | 'error' | 'no-controller'): void {
    if (!this.socket) {
      console.warn(`[LessonController] greeting_trigger送信不可: socket未初期化 reason=${reason}`);
      return;
    }
    const send = () => {
      this.socket.emit('greeting_trigger');
      console.log(`[LessonController] greeting_trigger送信 reason=${reason}`);
    };
    if (this.socket.connected) {
      send();
    } else {
      console.warn(`[LessonController] socket未接続、connect後に送信予約 reason=${reason}`);
